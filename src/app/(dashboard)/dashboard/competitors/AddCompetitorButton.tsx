    ...
    <div className="space-y-2">
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          style={{ background: "var(--ds-surface)", border: ".5px solid var(--ds-border)", color: "var(--ds-text)", fontSize: 14.3, paddingTop: 8, paddingBottom: 8 }}
          required
        />
        <button
          type="submit"
          ...
          style={{ background: "var(--ds-accent)", color: "var(--ds-text)", fontSize: 14.3, fontWeight: 500, padding: "8px 18px" }}
        >
          {loading ? (
            <>
      ...
      {error && (
        <p className="pl-1" style={{ color: "var(--ds-bad-soft)", fontSize: 12.1 }}>{error}</p>
      )}
    </div>
  )
}
