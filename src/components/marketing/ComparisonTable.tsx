  ...
  return (
    <section className="py-28 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <h2 className="text-[36px] sm:text-[52px] font-semibold text-fg leading-[1.1] tracking-tight text-center mb-16 text-balance">
        ...
        <div className="rounded-2xl border border-line/[0.08] overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={row.feature} className={`border-b border-line/[0.05] last:border-0 ${i % 2 === 0 ? "" : "bg-fg/[0.01]"}`}>
                  <td className="p-4 text-fg/60 text-sm">{row.feature}</td>
