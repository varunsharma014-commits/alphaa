    ...
    <section className="py-28 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
          {outcomes.map((o) => (
            <div key={o.title} className="rounded-2xl border border-line/[0.08] bg-fg/[0.03] hover:border-brand-orange/20 hover:bg-brand-orange/[0.03] transition-all duration-200 p-8">
              <p className="text-4xl mb-4">{o.icon}</p>
              <h3 className="text-fg font-semibold text-lg mb-3">{o.title}</h3>
              <p className="text-fg/50 text-sm leading-relaxed">{o.body}</p>
            </div>
          ))}
        </div>
        ...
        >
          ...
          <p className="text-fg/40 text-base mt-4 mb-8">14-day free trial. $0 today. Cancel anytime if it doesn't work.</p>
