...
export function Footer() {
  return (
    <footer className="bg-[#f5f5f7] text-[13.2px] leading-[1.33] tracking-[-0.01em] text-[#6e6e73]">
      <div className="max-w-[980px] mx-auto px-4 sm:px-6 pt-10 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="md:col-span-2">
            <Link href="/" className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[#1d1d1f] font-semibold text-[15.4px]">alphaa</span>
            </Link>
            <p className="text-[#6e6e73] text-[13.2px] leading-[1.5] max-w-xs mb-3">
              Get your business found on every AI search engine — ChatGPT, Claude, Gemini, and Perplexity. Automatically. $99/month.
            </p>
            <div className="flex flex-wrap gap-x-3 gap-y-1">
              {engines.map((e) => (
                <span key={e} className="text-[13.2px] text-[#86868b]">
                  {e}
                </span>
              ))}
            </div>
          ...
          {Object.entries(footerLinks).map(([section, links]) => (
            <div key={section}>
              <p className="text-[#1d1d1f] font-semibold text-[13.2px] mb-2.5">{section}</p>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-[#424245] text-[13.2px] hover:underline"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="border-t border-[#d2d2d7] pt-4 space-y-3">
          <div className="flex flex-col gap-2">
            <p className="text-[#6e6e73] text-[13.2px]">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-[#6e6e73] text-[13.2px]">Copyright © {new Date().getFullYear()} Alphaa. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
